...
export function HandwritingText({
  ...
  useEffect(() => {
    ...
    // rAF is the normal path and wins this race in any painting tab. But it does not
    // fire at all in a backgrounded tab or an offscreen renderer, and the word starts
    // fully transparent with its strokes dashed out of sight — so without a floor the
    // text would never appear at all, which is a worse failure than not animating.
    const floor = setTimeout(() => setDrawn(true), 200);
    return () => {
